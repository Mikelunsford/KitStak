...
| `receiving_orders` and `shipments` store lines as payload JSON | Normalised in migration 0050 (this session); JSON mirror kept in sync via handler dual-write until the multi-stage drop completes. | Closed (this session); multi-stage drop tracked in `03-workspace/journal/phase-7-lines-normalization.md`. |
