-- ============================================================================
-- Migration: 0100_job_runs_numbering.sql
-- Wave: 12
-- Phase: 3PL commercial pivot, Phase A6 (Job Runs and Daily Progress), step 3 of
--   4.
-- Closes: wires job_runs.run_number into the existing org-scoped numbering
--   chassis (numbering_sequences + next_doc_number from 0004, last extended in
--   0097). Prefix JR- per
--   03-workspace/specs/2026-06-13-3pl-a6-job-runs-handoff.md (decision 1: RUN-
--   collides with the spine production_run doc_type, so the job_run doc_type uses
--   JR-). The three-pl-api handler calls nextDocNumber on job-run create (A6 app
--   layer, next slice).
-- Date: 2026-06-14
--
-- DOWN MIGRATION (operator-only; not auto-run):
--   -- 1) Restore the 0097 doc_type CHECK (drops the job_run entry).
...
--   delete from public.numbering_sequences where doc_type = 'job_run';
--   -- 3) Restore the 0097 seed_org_numbering body. Operator copies from 0097.
--
...
--                      CONFLICT DO NOTHING for the seed). Mirrors 0097
--                      one-for-one.
--   State machine      Untouched.
--   Out of scope       The three-pl-api handler wiring is A6 app-layer code, not
--                      DDL. job_run_daily_logs have no numbering prefix.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Extend the doc_type CHECK to allow job_run. Drop and re-add with the full
--    enum (authoritative as of 0097) so the migration is reversible.
-- ---------------------------------------------------------------------------
...
alter table public.numbering_sequences
  ...
  check (doc_type in (
    ...
    'supply_plan',
    -- Wave 12 3PL commercial layer Job Run (this migration, 0100)
    'job_run'
  ));

comment on constraint numbering_sequences_doc_type_check on public.numbering_sequences is
  'Enumerates every doc_type allowed in numbering_sequences. Extended in 0100 to add job_run (JR-) for the 3PL Job Run (Wave 12). JR- is distinct from the production_run RUN- prefix.';

-- ---------------------------------------------------------------------------
-- 2) Seed a job_run row for every existing org. ON CONFLICT DO NOTHING keeps
--    the migration idempotent. Prefix JR-; pad_width 5; yearly reset.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) Extend seed_org_numbering (last redefined in 0097) to include job_run.
--    Body matches 0097 verbatim except for the two arrays (one new entry each).
-- ---------------------------------------------------------------------------
...
comment on function public.seed_org_numbering(uuid) is
  'Seeds numbering_sequences rows for every doc_type. Called by provision_organization. Idempotent. Extended in 0100 to seed job_run (JR-) for the 3PL Job Run (Wave 12).';
