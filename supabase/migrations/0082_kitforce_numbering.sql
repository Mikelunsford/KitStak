-- ============================================================================
-- Migration: 0082_kitforce_numbering.sql
-- Wave: 11
-- Phase: Pillar 4 (KitForce) foundation, step 5 of 5
-- Closes: wires workforce_members.member_number (EMP-), shifts (SHF-), and
--   work_assignments.assignment_number (WA-) into the existing org-scoped
--   numbering chassis (numbering_sequences + next_doc_number from 0004, last
--   extended in 0077). Implements Decision N1 of
--   03-workspace/specs/kitforce-pillar-spec.md (APPROVED 2026-05-31).
--   The kitforce-api handler calls nextDocNumber on create.
-- Date: 2026-05-31
--
-- DOWN MIGRATION (operator-only; not auto-run):
--   -- 1) Restore the 0077 doc_type CHECK (drops the three kitforce entries).
...
--     where doc_type in ('workforce_member', 'shift', 'work_assignment');
--
--   -- 3) Restore the 0077 seed_org_numbering body (drops the three kitforce
--   --    doc_types / prefixes). Operator copies from 0077.
--
...
--                      ON CONFLICT DO NOTHING for the seed). Re-runs are safe.
--                      Mirrors 0077 one-for-one.
--   State machine      Untouched.
--   Out of scope       The kitforce-api handler wiring (calling nextDocNumber on
--                      create) ships in the same PR but is application code, not
--                      DDL. time_entries has no numbering prefix (not in spec).
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Extend the numbering_sequences.doc_type CHECK to allow the three kitforce
--    doc_types. Drop and re-add with the full enum (authoritative as of 0077)
--    so the migration is reversible and explicit.
-- ---------------------------------------------------------------------------
...
comment on constraint numbering_sequences_doc_type_check on public.numbering_sequences is
  'Enumerates every doc_type allowed in numbering_sequences. Extended in 0082 to add workforce_member (EMP-), shift (SHF-), work_assignment (WA-) for the KitForce pillar (Wave 11).';

-- ---------------------------------------------------------------------------
-- 2) Seed a workforce_member / shift / work_assignment row for every existing
--    org. ON CONFLICT DO NOTHING keeps the migration idempotent.
--    Prefixes EMP- / SHF- / WA- per Decision N1; pad_width 5; yearly reset
...
-- ---------------------------------------------------------------------------
-- 3) Extend seed_org_numbering (last redefined in 0077) to include the three
--    kitforce doc_types. provision_organization calls this on every new org,
--    so without the update future orgs would miss the seed and the handler's
--    nextDocNumber call would auto-seed via next_doc_number's fallback insert,
--    losing our chosen prefix / pad_width / reset policy.
--
--    The function body matches 0077 verbatim except for the doc_types and
--    prefixes arrays (three new entries appended).
-- ---------------------------------------------------------------------------
...
comment on function public.seed_org_numbering(uuid) is
  'Seeds numbering_sequences rows for every doc_type. Called by provision_organization. Idempotent. Extended in 0082 to seed workforce_member (EMP-), shift (SHF-), work_assignment (WA-) for the KitForce pillar.';
