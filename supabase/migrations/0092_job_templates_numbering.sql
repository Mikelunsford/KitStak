-- ============================================================================
-- Migration: 0092_job_templates_numbering.sql
-- Wave: 12
-- Phase: 3PL commercial pivot, Phase A2 (Job Builder foundation), step 2 of 2
-- Closes: wires job_templates.template_number into the existing org-scoped
--   numbering chassis (numbering_sequences + next_doc_number from 0004, last
--   extended in 0090). Prefix JB- per
--   03-workspace/specs/2026-06-04-3pl-commercial-pivot-and-wms-pillar-plan.md
--   (Phase A2, section 6.1). The three-pl-api handler calls nextDocNumber on
--   job-template create.
-- Date: 2026-06-13
--
-- DOWN MIGRATION (operator-only; not auto-run):
--   -- 1) Restore the 0090 doc_type CHECK (drops the job_template entry).
...
--   delete from public.numbering_sequences where doc_type = 'job_template';
--
--   -- 3) Restore the 0090 seed_org_numbering body (drops the job_template
--   --    doc_type / prefix). Operator copies from 0090.
--
...
--                      Mirrors 0090 one-for-one.
--   State machine      Untouched.
--   Out of scope       The three-pl-api handler wiring (calling nextDocNumber on
--                      create) ships in the same Phase A2 PR but is application
--                      code, not DDL. job_template_lines has no numbering prefix
--                      (lines under a template, not a document).
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Extend the numbering_sequences.doc_type CHECK to allow the job_template
--    doc_type. Drop and re-add with the full enum (authoritative as of 0090) so
...
alter table public.numbering_sequences
  ...
  check (doc_type in (
    ...
    'three_pl_account',
    -- Wave 12 3PL commercial layer Job Builder (this migration, 0092)
    'job_template'
  ));

comment on constraint numbering_sequences_doc_type_check on public.numbering_sequences is
  'Enumerates every doc_type allowed in numbering_sequences. Extended in 0092 to add job_template (JB-) for the 3PL Job Builder (Wave 12).';

-- ---------------------------------------------------------------------------
-- 2) Seed a job_template row for every existing org. ON CONFLICT DO NOTHING
--    keeps the migration idempotent. Prefix JB-; pad_width 5; yearly reset
...
-- ---------------------------------------------------------------------------
-- 3) Extend seed_org_numbering (last redefined in 0090) to include the
--    job_template doc_type. provision_organization calls this on every new org,
...
--
--    The function body matches 0090 verbatim except for the doc_types and
...
comment on function public.seed_org_numbering(uuid) is
  'Seeds numbering_sequences rows for every doc_type. Called by provision_organization. Idempotent. Extended in 0092 to seed job_template (JB-) for the 3PL Job Builder (Wave 12).';
