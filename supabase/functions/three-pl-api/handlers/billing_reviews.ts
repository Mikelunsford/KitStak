...
// Workstream C (UI scan) server list toolbar allowlists. review_number is
// nullable (migration 0102: org-scoped, nullable, unique only where present) so
// it is a SEARCH column only, never a sort. SORT_COLS are NOT NULL only
// (created_at, status), so the keyset cursor never straddles a null. The money
// totals (estimate / actual) are nullable, so they are not sorts either.
// DEFAULT_SORT keeps the legacy created_at desc ordering when no sort param is
// sent.
...
export async function listBillingReviews({ req, url }: RouteCtx): Promise<Response> {
  ...
  const sort = parseSort(url, BILLING_REVIEW_SORT_COLS, BILLING_REVIEW_DEFAULT_SORT);
  ...
  if (search) q = q.or(buildSearchOr(BILLING_REVIEW_SEARCH_COLS, search));
  q = q
  ...
  const rows = (data ?? []).map((r) => BillingReviewSchema.parse(r)) as Array<
    ReturnType<typeof BillingReviewSchema.parse> & { id: string }
  >;
