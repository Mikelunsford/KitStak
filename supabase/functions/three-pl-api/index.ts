...
async function loadBillingReview(caller: Caller, id: string): Promise<BillingReview> {
  const { data, error } = await admin()
...
async function assertBillingReviewParent(caller: Caller, id: string): Promise<void> {
...
const TABLE: Route[] = [
  ...
  // -------------------------------------------------------------------------
  // billing_reviews (parent; Billing Review; FSM draft / approved / invoiced /
  // cancelled). approve / cancel are RPCs: approve cuts the spine DRAFT invoice
  // (it mints the INV- number itself, never passed from the wire) and moves
  // draft -> approved; cancel moves draft|approved -> cancelled. review_number
  // is BILL- (numbering chassis, 0103). Phase A7.
  // -------------------------------------------------------------------------
  {
    method: 'GET', path: '/billing-reviews',
    handler: async ({ req, url }) => {
      const caller = requireCaller(req);
      const status = url.searchParams.get('status');
  ...
  {
    method: 'POST', path: '/billing-reviews',
    handler: async ({ req }) => {
      const caller = requireCaller(req);
  ...
  {
    method: 'GET', path: '/billing-reviews/:id',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
      parseUuidParam(params.id);
  ...
  {
    method: 'PATCH', path: '/billing-reviews/:id',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
  ...
  {
    method: 'DELETE', path: '/billing-reviews/:id',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
  ...
  {
    method: 'POST', path: '/billing-reviews/:id/approve',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
  ...
  {
    method: 'POST', path: '/billing-reviews/:id/cancel',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
  ...
  // -------------------------------------------------------------------------
  // view_job_profitability (read-only view; Job Profitability, Phase A7). Read
  // via the service-role admin() client, which bypasses RLS, so the org filter
  // is applied EXPLICITLY here; the view's security_invoker is only a backstop
  // for non-admin readers.
  // -------------------------------------------------------------------------
  {
    method: 'GET', path: '/profitability',
    handler: async ({ req, url }) => {
      const caller = requireCaller(req);
  ...
  {
    method: 'GET', path: '/profitability/:jobRunId',
    handler: async ({ req, params }) => {
      const caller = requireCaller(req);
      requireCap(caller, 'threepl.profitability.read');
