...
/**
 * KittingJobDetailPage. Pillar 3. Migrated to the shared UI kit
 ...
 * STATE_STEPPER_PATHS, so there is no StateStepper. The consumed/produced line
 * tables keep their per-section fetch indicators and add forms, so they stay
 * hand-rolled tables rather than DataTables.
 *
      ...
      >
        <div className="flex gap-2 flex-wrap">
          ...
          {isStarted && caps.can('copack.kitting_job.complete') && (
            <Button variant="secondary" onClick={onComplete} disabled={complete.isPending}>
              Complete
            </Button>
          )}
